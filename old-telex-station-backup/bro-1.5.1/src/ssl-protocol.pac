...
type ApplicationData = record {
    cont : bytestring &length = $context.analyzer.current_record_length();
} &let {
	state_changed : bool =
...
type ServerKeyExchange = record {
    p_len :  uint16;
    p : bytestring &length = p_len;
    g_len : uint16;
    g : bytestring &length = g_len;
    pub_key_len : uint16;
    pub_key : bytestring &length = pub_key_len;
...
type SSLPDU =  case $context.analyzer.state() of { # record {
...
analyzer SSLAnalyzer {
	...
	function next_record(rec : const_bytestring, type : int,
        ...
        bro_event_ssl_record(bro_analyzer_, bro_analyzer_->Conn(), is_orig, type, bytestring_to_val(rec));
	...
	function transition(olds : AnalyzerState, news : AnalyzerState,
        ...
        bro_event_ssl_state_change(bro_analyzer_, bro_analyzer_->Conn(),
                                   string_to_val(state_label(old_state_)),
                                   string_to_val(state_label(news)));
		return true;
		%}

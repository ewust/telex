# $Id: hot.bro 6452 2008-12-07 01:19:13Z vern $
...
export {
	redef enum Notice += {
		SSH_Overflow,	# volume signature of SSH overflow attack seen
	};
...
# Detection of a particular SSHv1 overflow attack.
global ssh_notice_hosts: table[addr] of count &default = 0;

const ssh_overflow_sizes = { 102575, 235695, } &redef;

event connection_finished(c: connection)
	{
	if ( c$id$resp_p == ssh && c$orig$size in ssh_overflow_sizes )
		{
		if ( ++ssh_notice_hosts[c$id$orig_h] == 1 )
			{
			NOTICE([$note=SSH_Overflow,
				$conn=c,
				$msg=fmt("%d -> %d bytes",

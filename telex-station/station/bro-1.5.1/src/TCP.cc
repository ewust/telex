...
void TCP_Analyzer::DeliverPacket(int len, const u_char* data, bool is_orig,
	...
	endpoint->last_TTL = ip->TTL();
	endpoint->last_IP_ID = ip->IP_ID();
...
void TCP_Analyzer::UpdateEndpointVal(RecordVal* endp, int is_orig)
	...
	endp->Assign(2, new Val(s->LastSeq(), TYPE_COUNT));
	endp->Assign(3, new Val(s->window, TYPE_COUNT));
	endp->Assign(4, new Val(s->window_scale, TYPE_COUNT));
	endp->Assign(5, new Val(s->window_seq, TYPE_COUNT));
	endp->Assign(6, new Val(s->last_TTL, TYPE_COUNT));
	endp->Assign(7, new Val(s->last_IP_ID, TYPE_COUNT));
	}
...
int TCP_Analyzer::TCPOptionEvent(unsigned int opt,
	...
	if ( tcp_option )
		...
		vl->append(new StringVal(optlen, (const char *)option));
